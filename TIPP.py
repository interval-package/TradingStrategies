...
import numpy as np
import pandas as pd
from CPPI import getData
from myData import getSlicedGrowthPerData
...
def getParameters(test_num=1, rate_type=0, trading_year=1, trading_day_per_year=255, rf=0.04, init_nav=1e4,
                  adj_period=5, guarantee_rate=0.8, risk_multipler=2, risk_trading_fee_rate=0.01):
    """
    存储所需的固定金融参数初始化的数值
    ...
    : return: 用字典存储各个金融参数
    """
    parameters = {"test_num": test_num, "rate_type": rate_type, "trading_year": trading_year, "rf": rf,
                  "trading_day_per_year": trading_day_per_year, "rf_daily": rf / trading_day_per_year,
                  "trading_day_sum": trading_year * trading_day_per_year, "init_nav": init_nav,
                  "adj_period": adj_period, "guarantee_rate": guarantee_rate, "risk_multipler": risk_multipler,
                  "risk_trading_fee_rate": risk_trading_fee_rate}
...
def main():
    data = getData(raw_data=getSlicedGrowthPerData())
    # data = getData(filename='zhongzheng500.xlsx')
    Return = []
    for i in range(len(data)):

        p = getParameters(trading_day_per_year=data[i].shape[0], risk_trading_fee_rate=0.02)
